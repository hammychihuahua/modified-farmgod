ScriptAPI.register('FarmGod_Mod_Perfect', true, 'Warre', 'nl.tribalwars@coma.innogames.de');
window.FarmGod = {};
...
window.FarmGod.Translation = (function () {
  const msg = {
    int: {
      ...
      options: {
        title: 'FarmGod (Smart 1.5Hr Mod)',
        warning: '<b>คำแนะนำ:</b><br>- ติ๊กถูกช่องล่างสุดเพื่อส่งปุ่ม C ถ้ารายงานอายุ < 1.5 ชม.',
    ...
    let farmProcessor = ($html) => {
      if ($.isEmptyObject(data.farms.templates)) {
        ...
        $html.find('form[action*="action=edit_all"]').find('input[type="hidden"][name*="template"]').closest('tr').map((i, el) => {
          let $el = $(el);
          let classMatch = $el.prev('tr').find('a.farm_icon').first().attr('class').match(/farm_icon_([a-c])/);
          let tName = classMatch ? classMatch[1] : 'a';
          return (data.farms.templates[tName] = {
      ...
      let serverTimeMs = lib.getCurrentServerTime(); // แก้ปัญหา Timezone 
      ...
      $html.find('#plunder_list').find('tr[id^="village_"]').map((i, el) => {
        ...
        let diffHours = 999;
        if (timeText) {
            let now = new Date(serverTimeMs); // ใช้เวลา Server แทนเวลาเครื่องคอมพิวเตอร์
            let reportTime = new Date(now.getTime()); 
        ...
        let coordAnchor = $el.find('a[href*="screen=report&mode=all&view="]').first();
        if (!coordAnchor || coordAnchor.length === 0) return;
        
        let dotMatch = $el.find('img[src*="graphic/dots/"]').attr('src');
        let safeColor = dotMatch ? dotMatch.match(/dots\/(green|yellow|red|blue|red_blue)/)[1] : 'blue';

        return (data.farms.farms[coordAnchor.text().toCoord()] = {
